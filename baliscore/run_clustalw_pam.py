...
def run_clustalw(input_file: str, output_file: str):
    """
    Executa o ClustalW usando a matriz PAM padrão
    Args:
    ...
    cmd = [
        clustalw_path,
        f"-INFILE={input_file}",
        "-PWMATRIX=pam",     # Matriz PAM para alinhamento par a par
        "-MATRIX=pam",       # Matriz PAM para alinhamento múltiplo
        "-ALIGN",
        "-OUTPUT=FASTA",
    ...
    try:
        logging.info(f"Executando ClustalW com matriz PAM padrão...")
...
def process_all_files(input_dir: str, output_dir: str):
    ...
    for i, input_file in enumerate(input_files, 1):
        try:
            ...
            output_file = str(Path(output_dir) / f"{input_path.stem}_clustalw_pam.fasta")
            
            logging.info(f"Processando arquivo {i}/{total_files}: {input_path.stem}")
            
            if run_clustalw(input_file, output_file):
                successful += 1
            else:
                failed += 1
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 run_clustalw_pam.py <input_dir> <output_dir>")
        print("Example: python3 run_clustalw_pam.py /home/augusto/projects/multiOne/BAliBASE/RV100 ./clustalw_pam_results")
        sys.exit(1)
